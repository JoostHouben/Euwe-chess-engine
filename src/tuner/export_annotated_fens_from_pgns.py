...
def get_annotated_fens_for_game(game: Game) -> tuple[list[AnnotatedFen], int]:
    ...
    while node is not None:
        ...
        played_move = child.move
        played_move_is_capture = node.board().is_capture(played_move)
        ...
        if include_node:
            ...
            annotated_fens.append(
                AnnotatedFen(
                    ...
                    move_is_capture=played_move_is_capture,
                )
            )
        else:
            num_filtered += 1
...
def export_annotated_pgns(pgn_paths: list[Path], out_path: Path) -> None:
    ...
    pool_results = multiprocessing.Pool().imap_unordered(
        get_annotated_fens_for_game, merged_games, chunksize=10
    )
